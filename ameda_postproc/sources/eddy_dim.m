...
% (contains at least xy_cj and xy_ci)  %MODIF abarboni
...
if ~isempty(eddy_lim{3})
    ...
    % compute deta3
    in_eddy = inpolygon(x,y,xy{3}(1,:),xy{3}(2,:)); %MODIF abarboni
    ...
    if ~isempty(eddy_lim{1})
        in_eddy = inpolygon(x,y,xy{1}(1,:),xy{1}(2,:)); % MODIF abarboni
        if type_c == -1 % anticylone case
    ...
    if ~isempty(eddy_lim{2})
        in_eddy = inpolygon(x,y,xy{2}(1,:),xy{2}(2,:)); %MODIF abarboni
        if type_c == -1 % anticylone case
